...
Algorithm Style: Implemented using standard arrays for tracking distances and visited nodes with simple loops ($O(V^2)$ time complexity) instead of a priority queue, adhering strictly to the bonus task notes.
Overflow Prevention: The algorithm explicitly checks for `Integer.MAX_VALUE` before relaxation to prevent arithmetic overflow when adding weights to unvisited nodes.
